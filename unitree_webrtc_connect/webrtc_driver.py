...
SET_REMOTE_DESCRIPTION_TIMEOUT = float(os.environ.get("UNITREE_SET_REMOTE_TIMEOUT", "15"))
FILTER_REMOTE_CANDIDATES = os.environ.get("UNITREE_FILTER_REMOTE_CANDIDATES", "1").lower() not in {"0", "false", "no"}
...
class UnitreeWebRTCConnection:
    ...
    async def init_webrtc(self, turn_server_info=None, ip=None):
        ...
        answer_sdp = peer_answer["sdp"]
        if FILTER_REMOTE_CANDIDATES and self.connectionMethod in {WebRTCConnectionMethod.LocalSTA, WebRTCConnectionMethod.LocalAP}:
            answer_sdp = self._filter_remote_sdp_candidates(answer_sdp, self.ip)

        remote_sdp = RTCSessionDescription(sdp=answer_sdp, type=peer_answer['type']) 
        logging.info(
            "Applying remote SDP answer: type=%s sdp_bytes=%d",
            peer_answer.get("type"),
            len(answer_sdp),
        )
        try:
            await asyncio.wait_for(
                self.pc.setRemoteDescription(remote_sdp),
                timeout=SET_REMOTE_DESCRIPTION_TIMEOUT,
            )
        except asyncio.TimeoutError as exc:
            raise TimeoutError(
                f"setRemoteDescription timed out after {SET_REMOTE_DESCRIPTION_TIMEOUT}s "
                f"(peer={self.pc.connectionState}, ice={self.pc.iceConnectionState}, "
                f"signaling={self.pc.signalingState})"
            ) from exc
        logging.info(
            "Remote SDP applied: peer=%s ice=%s signaling=%s channel=%s",
            self.pc.connectionState,
            self.pc.iceConnectionState,
            self.pc.signalingState,
            self.datachannel.channel.readyState,
        )

        reporter = asyncio.create_task(self._report_connect_state())
        try:
    ...
    async def _report_connect_state(self):
        while True:
            await asyncio.sleep(3)
            pc = self.pc
            channel = getattr(getattr(self, "datachannel", None), "channel", None)
            logging.info(
                "WebRTC connect state: peer=%s ice=%s signaling=%s channel=%s validated=%s",
                getattr(pc, "connectionState", "?"),
                getattr(pc, "iceConnectionState", "?"),
                getattr(pc, "signalingState", "?"),
                getattr(channel, "readyState", "?"),
                getattr(getattr(self, "datachannel", None), "data_channel_opened", False),
            )

    def _filter_remote_sdp_candidates(self, sdp: str, allowed_ip: str) -> str:
        if not allowed_ip:
            return sdp
        ...
        for line in sdp.splitlines():
            if line.startswith("a=candidate:"):
                parts = line.split()
                candidate_ip = parts[4] if len(parts) > 4 else ""
                if candidate_ip != allowed_ip:
                    dropped += 1
                    continue
                kept += 1
            lines.append(line)
        ...
        if dropped:
            logging.info(
                "Filtered remote ICE candidates: kept=%d dropped=%d allowed_ip=%s",
                kept,
                dropped,
                allowed_ip,
            )
