...
class WebRTCAudioHub:
    ...
    def _log_upload_chunk(self, api_name, parameter):
        """Log upload progress without dumping base64 audio payloads."""
        self.logger.debug(
            "%s chunk %s/%s size=%s total_file_size=%s md5=%s name=%s",
            api_name,
            parameter.get("current_block_index"),
            parameter.get("total_block_number"),
            parameter.get("current_block_size"),
            parameter.get("file_size"),
            parameter.get("file_md5"),
            parameter.get("file_name"),
        )
    ...
    async def upload_audio_file(self, audiofile_path):
        ...
        try:
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                self._log_upload_chunk("UPLOAD_AUDIO_FILE", parameter)
                # Send the chunk
    ...
    async def upload_megaphone(self, audiofile_path):
        ...
        try:
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                self._log_upload_chunk("UPLOAD_MEGAPHONE", parameter)
                # Send the chunk
                self.logger.info(f"Sending chunk {i}/{total_chunks}")
