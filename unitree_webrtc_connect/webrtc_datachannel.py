...
class WebRTCDataChannel:
    ...
    async def wait_datachannel_open(self, timeout=None):
        ...
        if timeout is None:
            timeout = float(os.environ.get("UNITREE_WEBRTC_TIMEOUT", "15"))
        reporter = asyncio.create_task(self._report_wait_state())
        try:
    ...
    async def _report_wait_state(self):
        while True:
            await asyncio.sleep(3)
            pc = getattr(self.conn, "pc", None)
            logging.info(
                "Waiting datachannel: peer=%s ice=%s signaling=%s channel=%s validated=%s",
                getattr(pc, "connectionState", "?"),
                getattr(pc, "iceConnectionState", "?"),
                getattr(pc, "signalingState", "?"),
                getattr(self.channel, "readyState", "?"),
