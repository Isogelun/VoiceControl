...
def make_local_request(path, body=None, headers=None, timeout=None):
    """LAN-side POST. Returns the response object on success, None otherwise."""
    if timeout is None:
        timeout = float(os.environ.get("UNITREE_SIGNALING_TIMEOUT", "5"))
    try:
        logging.info("LAN signaling POST %s", path)
        response = requests.post(url=path, data=body, headers=headers, timeout=timeout)
...
def send_sdp_to_local_peer_new_method(ip, sdp, aes_128_key: str = None):
    ...
    try:
        ...
        logging.info("LAN signaling con_notify OK")
        ...
        if response:
            ...
            logging.info("LAN signaling con_ing_%s OK", path_ending)
